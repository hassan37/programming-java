...
public class _2_LeftRotation {
	...
	public int[] rotLeft(int[] a, int d) {
		...
		int factor = (totalElements - rotations);
		for (int i = 0; i < totalElements; i++) {
			int e = a[i];
			int eIndex = (i + factor) % totalElements;
...
// 2. Feature to be tested (recommended)
	...
	public void should_returnNullArray_When_NullArrayPassed() {
		...
		_2_LeftRotation lr = new _2_LeftRotation();
	...
	public void withinRangeLeftRotationsOnElements() {
		...
		assertArrayEquals(new int[] {2,3,4,5,1}, lr.rotLeft(arr, 1));
		assertArrayEquals(new int[] {3,4,5,1,2}, lr.rotLeft(arr, 2));
		assertArrayEquals(new int[] {4,5,1,2,3}, lr.rotLeft(arr, 3));
		assertArrayEquals(new int[] {5,1,2,3,4}, lr.rotLeft(arr, 4));
	...
	public void outOfRangeLeftRotationsOnElements() {
		...
		assertArrayEquals(new int[] {2,3,4,5,1}, lr.rotLeft(arr, 100006));
		assertArrayEquals(new int[] {3,4,5,1,2}, lr.rotLeft(arr, 200007));
		assertArrayEquals(new int[] {4,5,1,2,3}, lr.rotLeft(arr, 300008));
		assertArrayEquals(new int[] {5,1,2,3,4}, lr.rotLeft(arr, 400009));
		assertArrayEquals(new int[] {1,2,3,4,5}, lr.rotLeft(arr, 500010));
	}
}
